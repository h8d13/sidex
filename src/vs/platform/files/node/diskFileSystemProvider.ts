...
export class DiskFileSystemProvider extends AbstractDiskFileSystemProvider implements
	...
	private toType(entry: { isFile(): boolean; isDirectory(): boolean; isSymbolicLink?(): boolean }, symbolicLink?: { dangling: boolean }): FileType {
	...
	async readFile(resource: URI, options?: IFileAtomicReadOptions): Promise<Uint8Array> {
		...
		try {
			...
			const data: number[] = await invoke('fs_read_file_bytes', { path: filePath });
			...
			if (isFileOpenForWriteOptions(opts) && opts.unlock) {
				try {
					...
					if (!(stat.mode & 0o200 /* File mode indicating writable by owner */)) {
						await invoke('fs_chmod', { path: filePath, mode: stat.mode | 0o200 });
					}
				} catch (error) {
	...
	async mkdir(resource: URI): Promise<void> {
		try {
			await invoke('fs_mkdir', { path: this.toFilePath(resource) });
		} catch (error) {
			throw this.toFileSystemProviderError(error);
		}
	}
	...
	async delete(resource: URI, opts: IFileDeleteOptions): Promise<void> {
		try {
			...
			if (opts.recursive) {
				...
				try {
					await invoke('fs_unlink', { path: filePath });
				} catch (unlinkError: any) {

					if (unlinkError.code === 'EPERM' || unlinkError.code === 'EISDIR' || String(unlinkError).includes('EPERM') || String(unlinkError).includes('EISDIR')) {
						...
						if (isDirectory) {
							await invoke('fs_rmdir', { path: filePath });
						} else {
							throw unlinkError;
						}
					} else {
	...
	private async doCloneFile(from: URI, to: URI, mkdir: boolean): Promise<void> {
		...
		try {
			...
			if (mkdir) {
				await invoke('fs_mkdir_recursive', { path: dirname(toFilePath) });
			}

			await invoke('fs_copy_file', { source: fromFilePath, target: toFilePath });
		} catch (error) {
